...
genome_dir=/scratch/genomes/hg38/bismark
min_insert=0
max_insert=2000
cpus=$(( $CPU / 2 ))
...
### OUTPUT
outdir=${workdir}/2_bismark
tmp_dir=/tmp

# 1. bismark PE files
log_bismark_pe=${outdir}/${base}.bismark.log
bam_pe=${outdir}/${base}_bismark_bt2_pe.bam
read1_unmapped_fq=${outdir}/${base}_R1.fq.gz_unmapped_reads_1.fq.gz
read2_unmapped_fq=${outdir}/${base}_R2.fq.gz_unmapped_reads_2.fq.gz
read_se1_fq=${outdir}/${base}_R1.fq.gz
read_se2_fq=${outdir}/${base}_R2.fq.gz
...
#mv $read1_unmapped_fq $read_se1_fq
#mv $read2_unmapped_fq $read_se2_fq
echo ""
...
echo "-- 3. Analyse methylation in $bam_dedup_pe using $cpus threads... started on $(date)"
